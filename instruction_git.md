...
чтобы увидеть как создавались и сливались ветки нужно использовать "флаг":

    git log --graph

лучше его использовать с уже известными нам "флагами" --all и --oneline
...
В случае, если при слиянии веток, Git обнаружит, что строки с одним номером отличаются, он выдас ошибку и предложит пользователю выбрать правильный вариант, такая ситуация` называется merge-conflict
